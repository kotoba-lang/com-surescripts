(ns surescripts.main
  "Kotodama WASM entrypoint for the Surescripts clean-room actor (L4) — Clojure port.
  ...
  facts (`surescripts.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "surescripts")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "surescri_pre"
   ...
   {:entity "Claim" :plural "claims" :id-prefix "surescri_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "surescri_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "surescri_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "surescri_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {:memberId "Member"}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "surescri_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"surescripts.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "surescripts-compat" :tier tier :entities entities} 200])
...
(defn start! [] :surescripts-compat/ready)
